...
const pct1 = (num: number | null, den: number) => (num === null ? null : den ? Math.round((num / den) * 1000) / 10 : 0);
...
export function GraphView({ funnel, selectedStep, zoomChapter, onSelectStep, onZoom }: GraphViewProps) {
  ...
  const chapterBars: FunnelBar[] = funnel.chapters.map((c, i) => ({ id: c.key, label: c.short, pct: pct1(c.exit, START), prevPct: i ? pct1(funnel.chapters[i - 1].exit, START) ?? undefined : 100, chapter: c.key === 'paywall' ? 'paywall' : 'x' }));
    ...
    <div style={{ display: 'grid', gap: 12 }}>
      <Card padding="wide" style={{ minWidth: 0, overflow: 'hidden' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6, gap: 12 }}>
          {zc ? <button type="button" onClick={() => { onZoom(null); onSelectStep(null); }} style={{ background: 'none', border: 0, padding: 0, cursor: 'pointer', font: '600 13px/1.4 var(--font-sans)', color: 'var(--ink-600)' }}>‹ All chapters</button> : <p style={{ margin: 0, font: '600 13px/1.4 var(--font-sans)' }}>{START} {funnel.startedSource === 'accounts_created' ? 'accounts' : 'started'} · by chapter</p>}
        ...
        <p style={{ margin: '4px 0 0', font: '400 11px/1.5 var(--font-sans)', color: 'var(--text-tertiary)' }}>{zc ? 'Each bar is one step · % of the ' + START + ' ' + STARTED_NOUN[funnel.startedSource] : '% of the ' + START + ' still in at the end of each chapter · hatched = left'}{funnel.untrackedSteps ? ' · ' + UNTRACKED_NOTE : ''}</p>
      </Card>
      {!zc && <Card padding="none"><div style={{ marginTop: -1 }}>{funnel.chapters.map(c => <Row key={c.key} title={c.label} sub={count(c.enter) + ' came in · ' + count(c.exit) + ' finished · ' + c.steps.length + ' steps'} pct={sharePct(c.exit, c.enter)} lost={c.enter === null || c.exit === null ? null : c.enter - c.exit} green={c.key === 'paywall'} onClick={() => zoomTo(c.key)} />)}</div></Card>}
