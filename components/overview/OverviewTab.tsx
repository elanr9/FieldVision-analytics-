...
export function OverviewTab({ months, weeks, totals, usage, push }: OverviewTabProps) {
      ...
      <Fade id={view}>
        ...
        {view === 'usage' && <div style={{ display: 'grid', gap: 12 }}>
          ...
          <Card padding="wide" style={{ paddingTop: 6, paddingBottom: 6 }}>
            ...
            {features.slice(0, 6).map(f => <UsageBar key={f.id} f={f} max={maxU} onClick={() => push({ key: 'usage', node: <UsageScreen features={usage.detail} focus={f.id} /> })} />)}
